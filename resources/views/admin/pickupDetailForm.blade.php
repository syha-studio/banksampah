            ...
            <form action="/pick/{{ $pickup->id }}" method="post">
                ...
                <div class="grid gap-4 mb-10" id="pickup-details{{ $pickup->id }}">
                    <div class="pickup-detail{{ $pickup->id }} flex items-center space-x-4">
                        <select name="category_0" id="category_0" class="@error('category_0') is-invalid @enderror bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-design-primary focus:border-design-primary block w-auto p-2.5 category-select">
                            <option value="">Select Category</option>
                            @foreach ($categoriesOptions as $category)
                                <option value="{{ $category->id }}">{{ $category->name }}</option>
                            @endforeach
                        </select>
                        <select name="waste_0" id="waste_0" class="@error('waste_0') is-invalid @enderror bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-design-primary focus:border-design-primary block w-auto p-2.5 waste-select">
                            <option value="">Select Waste</option>
                        </select>
                        <div id="waste_price_0" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-48 p-2.5 waste-price-unit">Rp - / -</div>
                        <input type="number" name="qty_0" id="qty_0" placeholder="Qty" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-24 p-2.5 qty-input" min="0" step="1"/>
                        <div id="total_0" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-48 p-2.5 total">Total : Rp -</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelector('.add-row' + pickupId).addEventListener('click', function () {
            ...
            newRow.innerHTML = `
                <select name="category_${detailIndex}" id="category_${detailIndex}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-design-primary focus:border-design-primary block w-auto p-2.5 category-select">
                    <option value="">Select Category</option>
                    @foreach ($categoriesOptions as $category)
                        <option value="{{ $category->id }}">{{ $category->name }}</option>
                    @endforeach
                </select>
                <select name="waste_${detailIndex}" id="waste_${detailIndex}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-design-primary focus:border-design-primary block w-auto p-2.5 waste-select">
                    <option value="">Select Waste</option>
                </select>
                ...
                <input type="number" name="qty_${detailIndex}" id="qty_${detailIndex}" placeholder="Qty" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-24 p-2.5 qty-input" min="0" step="1"/>
